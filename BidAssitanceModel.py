    ...
    def _node_extract(self, state: GraphState) -> GraphState:
        ...
        sys = SystemMessage(
            content=(
                ...
                "제출서류(document_requirements)는 다음 3단계 우선순위로 작성하라:\n\n"
                "1단계) 공고문에서 명시적으로 찾기:\n"
                "   - 키워드: '입찰참가자격', '참가자격', '자격요건', '실적', '제출서류', '첨부서류' 등\n"
                "   - 명시된 내용이 있으면 정확히 추출하라\n\n"
                "2단계) 공고 유형으로 합리적 추론:\n"
                "   - 공사 (예: '수변녹지 경관개선 공사', '○○ 건축공사'):\n"
                "     * 참가자격: 건설업 면허 (토목/조경/건축 등 관련 업종)\n"
                "     * 실적: 최근 3년 이내 유사 공사실적\n"
                "     * 서류: 건설업 등록증, 실적증명서, 입찰참가신청서\n"
                "   - 용역 (예: '○○ 연구용역', '○○ 컨설팅'):\n"
                "     * 참가자격: 관련 업종 등록, 기술인력 보유\n"
                "     * 실적: 유사 용역 수행실적\n"
                "     * 서류: 사업자등록증, 기술인력 보유 현황\n"
                "   - 물품 (예: '○○ 물품 구매'):\n"
                "     * 참가자격: 제조업체 또는 판매업체\n"
                "     * 실적: 납품실적\n"
                "     * 서류: 제조자증명서, 품질인증서\n\n"
                "3단계) 정말 추론 불가능하면 빈 리스트 []\n\n"
                "제목, 금액, 내용을 종합적으로 보고 가장 합리적인 요건을 작성하라."
            )
        )
    ...
    def _node_report(self, state: GraphState) -> GraphState:
        ...
        sys = SystemMessage(
            content=(
                ...
                "   - qualification_requirements, performance_requirements, document_requirements에서 "
                "추출된 항목들을 체크박스 형식으로 작성하라. 예: '- [ ] 건설업 면허 보유자'\n"
                "   - 만약 모든 리스트가 비어있다면:\n"
                "     '⚠️ 공고문 본문에서 자격요건을 찾지 못했습니다.\n"
                "     첨부된 공사시방서, 입찰공고문, 특수조건 파일을 반드시 확인하세요.\n"
                "     일반적으로 필요한 서류: 입찰참가신청서, 사업자등록증, 이행(지급)보증증권, 실적증명서 등'\n"
                "# 3. 낙찰가 예측(범위/포인트/근거/)\n"
