    ...
    def _node_extract(self, state: GraphState) -> GraphState:
        ...
        sys = SystemMessage(
            content=(
                ...
                "중요: 다음 3가지를 공고문의 해당 섹션에서만 정확히 추출하라:\n\n"
                "1) qualification_requirements (참가자격):\n"
                "   - 공고문에서 다음과 같은 섹션 제목을 찾아라:\n"
                "     * '입찰참가자격', '가. 입찰참가자격', '참가자격 제한', '자격요건'\n"
                "   - 그 섹션 제목 **바로 아래에 나열된 항목만** 추출\n"
                "   - 예시 구조:\n"
                "     가. 입찰참가자격\n"
                "       1) 건설업 면허 보유자  ← 이것만 추출\n"
                "       2) 조경공사업 등록업체  ← 이것만 추출\n"
                "   - 해당 섹션이 없거나 비어있으면 빈 리스트 []\n"
                "   - ❌ 제목/다른 섹션에서 추론 절대 금지\n\n"
                "2) performance_requirements (실적요건):\n"
                "   - 공고문에서 '실적', '계약실적', '시공실적' 섹션 찾기\n"
                "   - 또는 '입찰참가자격' 섹션 내부에서 '실적' 관련 항목만\n"
                "   - 예: '최근 3년 이내 유사공사 실적 1건 이상'\n"
                "   - 없으면 빈 리스트 []\n\n"
                "3) document_requirements (제출서류):\n"
                "   - 공고문에서 다음 섹션 제목을 찾아라:\n"
                "     * '제출서류', '나. 제출서류', '첨부서류', '구비서류'\n"
                "   - 그 섹션 **바로 아래에 나열된 서류만** 추출\n"
                "   - 항상 기본 서류 포함:\n"
                "     * 입찰참가신청서\n"
                "     * 사업자등록증\n"
                "   - 공고문에 명시된 추가 서류 모두 포함\n"
                "   - 예: ['입찰참가신청서', '사업자등록증', '건설업등록증', '실적증명서']\n\n"
                "핵심 원칙:\n"
                "- 섹션 제목(예: '가. 입찰참가자격') 아래 내용만 보라\n"
                "- 다른 곳에서 추론/추측 금지"
            )
        )
    ...
    def _node_report(self, state: GraphState) -> GraphState:
        ...
        sys = SystemMessage(
            content=(
                ...
                "   ## 가. 참가자격 요건\n"
                "   - qualification_requirements 리스트의 각 항목을 한 줄씩 표시\n"
                "   - 체크박스 없이 '- 항목명' 형식\n"
                "   - 예: '- 건설업 면허 보유자'\n"
                "   - 비어있으면: '공고문에 명시된 자격요건 없음'\n\n"
                "   ## 나. 실적 요건\n"
                "   - performance_requirements 리스트의 각 항목을 한 줄씩 표시\n"
                "   - 예: '- 최근 3년 이내 유사공사 실적 1건 이상'\n"
                "   - 비어있으면: '공고문에 명시된 실적요건 없음'\n\n"
                "   ## 다. 제출서류\n"
                "   - document_requirements 리스트의 각 항목을 한 줄씩 표시\n"
                "   - 예: '- 입찰참가신청서', '- 사업자등록증'\n"
                "   - 비어있으면: '공고문에서 확인 필요 (일반적으로 입찰참가신청서, 사업자등록증은 필수)'\n\n"
                "# 3. 낙찰가 예측(범위/포인트/근거/)\n"
                "   - 이 섹션에는 반드시 '### 사정율 구간에 따른 상위 3개의 확률'이라는 소제목을 포함하라.\n"
                "   - top_ranges의 각 항목에서 'rate'(사정율 값)와 'probability'(확률)을 사용하라.\n"
                "   - 형식: '1순위: 사정율 {rate:.2f}% (확률 {probability:.2f}%)'\n"
                "   - 예시: '1순위: 사정율 3.45% (확률 29.57%)'\n"
                "   - range_display 값도 참고용으로 표시 가능: '1순위: 사정율 3.45% (구간: 3.4%~3.5%, 확률 29.57%)'\n"
