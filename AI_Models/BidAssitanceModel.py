...
class FeatureSeqDataset(Dataset):
    def __init__(self, X: np.ndarray, y: np.ndarray, target_log: bool = False):
        """
        X: (N, F) -> (N, F, 1)
        y: (N,)
        """
        ...
        if self.X.ndim != 2:
            raise ValueError(f"X must be 2D (N,F). Got shape={self.X.shape}")
        if self.y.ndim != 1:
            raise ValueError(f"y must be 1D (N,). Got shape={self.y.shape}")
    ...
    def __getitem__(self, idx: int):
        ...
        if self.target_log:
            if y < 0:
                raise ValueError(f"Target is negative (낙찰가 < 0): {y}")
            y = math.log1p(y)
...
# Model: RNN -> last hidden -> reg head
...
class RNNRegressor(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        _, h_n = self.rnn(x) 
        last = h_n[-1] 
        y_hat = self.head(last).squeeze(-1) 
        return y_hat
...
def run_training_rnn(
    ...
    if len(work) < 30:
        raise ValueError(f"유효 데이터가 너무 적습니다: {len(work)} rows (권장: 최소 수십~수백)")

    X = work[list(feature_cols)].to_numpy(dtype=np.float32)
    y = work[target_col].to_numpy(dtype=np.float32)

    # 2) split
    train_idx, val_idx, test_idx = split_indices(len(work), test_ratio=test_ratio, val_ratio=val_ratio, seed=seed)
    ...
    # 4) Dataset/Loader
    train_ds = FeatureSeqDataset(X_train_s, y_train, target_log=target_log)
    val_ds = FeatureSeqDataset(X_val_s, y_val, target_log=target_log)
    test_ds = FeatureSeqDataset(X_test_s, y_test, target_log=target_log)
    ...
    model = RNNRegressor(hidden_size=32, num_layers=1, dropout=0.0).to(device)
    ...
    for epoch in range(1, epochs + 1):
        ...
        yv_true, yv_pred = predict(model, val_loader, device)
        if target_log:
            yv_true_amt = np.expm1(yv_true)
            yv_pred_amt = np.expm1(yv_pred)
            val_m = regression_metrics(yv_true_amt, yv_pred_amt)
        else:
            val_m = regression_metrics(yv_true, yv_pred)

        row = {"epoch": float(epoch), "train_loss": float(tr_loss), **{f"val_{k}": v for k, v in val_m.items()}}
        history.append(row)
        ...
        improved = (best_val_rmse - val_m["RMSE"]) > min_delta
        if improved:
    ...
    yt_true, yt_pred = predict(model, test_loader, device)
    if target_log:
        yt_true_amt = np.expm1(yt_true)
        yt_pred_amt = np.expm1(yt_pred)
        test_m = regression_metrics(yt_true_amt, yt_pred_amt)
    else:
        test_m = regression_metrics(yt_true, yt_pred)

    print(
        f"[FINAL TEST] RMSE={test_m['RMSE']:.4f} | MAE={test_m['MAE']:.4f} | MAPE={test_m['MAPE']:.2f}"
    )
