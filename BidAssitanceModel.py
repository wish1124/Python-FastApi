    ...
    def _node_extract(self, state: GraphState) -> GraphState:
        ...
        sys = SystemMessage(
            content=(
                ...
                "제출서류(document_requirements)는 공고문에서 명시된 내용을 정확히 찾아서 리스트로 작성하라. "
                "예: ['건설업 면허 보유자', '최근 3년간 유사실적 1건 이상'], ['입찰참가신청서', '사업자등록증', '이행(지급)보증증권'] 등. "
                "공고문에 해당 정보가 없으면 빈 리스트 []로 반환하라."
            )
        )
    ...
    def _node_report(self, state: GraphState) -> GraphState:
        ...
        sys = SystemMessage(
            content=(
                ...
                "체크박스 형식으로 작성하라. 예: '- [ ] 건설업 면허 보유자', '- [ ] 최근 3년간 유사실적 1건 이상'\n"
                "   - 만약 해당 정보가 빈 리스트([])이거나 없다면, '공고문에 명시된 자격요건이 없습니다. 일반적인 공공입찰 기본요건(사업자등록증, 입찰참가신청서 등)을 확인하세요.'라고 작성하라.\n"
                "# 3. 낙찰가 예측(범위/포인트/근거/)\n"
                # --- 추가 및 수정된 부분 ---
